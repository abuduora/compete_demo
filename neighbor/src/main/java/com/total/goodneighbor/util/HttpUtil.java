...
public class HttpUtil {
    ...
    public static String get(String callUrl, Map<String, String> headers) {
        String resp = ""; // 应答内容
        try {
            System.out.println( "请求地址："+callUrl);
            ...
            // 打印HTTP调用的应答状态码和应答报文
            System.out.println( String.format("应答状态码=%d, 应答报文=%s", conn.getResponseCode(), resp) );
    ...
    // 从指定url获取图片
    public static Blob getImage(String callUrl, Map<String, String> headers) {
        Blob bitmap = null; // 位图对象
        try {
            System.out.println("请求地址："+callUrl);
            compatibleSSL(callUrl); // 兼容https开头的调用地址
            URL url = new URL(callUrl); // 根据网址字符串构建URL对象
            ...
            System.out.println(String.format("应答内容长度=%d, 内容类型=%s, 压缩方式=%s",
                    conn.getContentLength(), conn.getContentType(), conn.getContentEncoding()) );
            // 对输入流中的数据解码，得到位图对象
            //bitmap = BlobFactory.decodeStream(conn.getInputStream());
            // 打印图片获取的应答状态码和位图大小
            System.out.println( String.format("应答状态码=%d, 位图大小=%s", conn.getResponseCode()) );
            conn.disconnect(); // 断开连接
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static String post(String callUrl, String req, Map<String, String> headers) {
        String resp = ""; // 应答内容
        try {
            System.out.println("请求地址："+callUrl+", 请求报文="+req);
            //compatibleSSL(callUrl); // 兼容https开头的调用地址
            URL url = new URL(callUrl); // 根据网址字符串构建URL对象
            // 打开URL对象的网络连接，并返回HttpURLConnection连接对象
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
    ...
    public static String post(String callUrl, Blob req, Map<String, String> headers) {
        String resp = ""; // 应答内容
        try {
            System.out.println("请求地址："+callUrl+", 请求报文="+req);
            //compatibleSSL(callUrl); // 兼容https开头的调用地址
            ...
            conn.connect(); // 开始连接
            OutputStream os = conn.getOutputStream(); // 从连接对象中获取输出流
            os.write(req.getBytes(1,0)); // 往输出流写入请求报文
            // 打印HTTP调用的应答内容长度、内容类型、压缩方式
            System.out.println(String.format("应答内容长度=%s, 内容类型=%s, 压缩方式=%s",
                    conn.getHeaderField("Content-Length"), conn.getHeaderField("Content-Type"),
                    conn.getHeaderField("Content-Encoding")) );
    ...
    public static String upload(String uploadUrl, String uploadFile, Map<String, String> headers) {
        ...
        try (FileInputStream fis = new FileInputStream(uploadFile)) {
            System.out.println("上传地址："+uploadUrl+", 上传文件="+uploadFile);
